...
if env_info["has_secrets"]:
    ...
    st.info("💻 **Локальный режим**: Настройки сохраняются в session_state")

st.markdown("---")

# Показываем поля ввода только в локальном режиме
if not env_info["has_secrets"]:
    ...
    # В продакшене показываем только статус настроек
    st.markdown("### 📋 Статус настроек")
    secrets_status = env_info["configured_secrets"]
    for setting, is_set in secrets_status.items():
        status = "✅ Установлен" if is_set else "❌ Не установлен"
        st.write(f"**{setting}:** {status}")

# Показываем кнопки только в локальном режиме
if not env_info["has_secrets"]:
